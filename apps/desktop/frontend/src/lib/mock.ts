...
export const mockApi = {
  ...
  SetRegion: async (ref: string, region: string) => { const x = state.sessions.find((s) => s.id === ref); if (x) x.region = region; emit(); },
  SetProfile: async (ref: string, profile: string) => { const x = state.sessions.find((s) => s.id === ref); if (x && x.aws) (x.aws as unknown as { profile?: string }).profile = profile || undefined; emit(); },
