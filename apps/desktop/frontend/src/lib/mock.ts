...
function seed() {
  const sso: Integration = {
    ...
    awsSso: {
      ...
      region: "us-east-1",
      tokenExpires: new Date(Date.now() + 6e6).toISOString(),
    },
  } as Integration;
  const az: Integration = {
    id: "contoso",
    alias: "contoso",
    cloud: "azure",
    azure: { tenantId: "t-1", account: "nate@contoso.com" },
  } as Integration;
  const gcp: Integration = {
    id: "gcp",
    alias: "gcp",
    cloud: "gcp",
  ...
  const out: Integration = {
    ...
    alias: "acme-eu",
    cloud: "aws",
    awsSso: { startUrl: "https://acme-eu.awsapps.com/start", region: "eu-west-1", tokenExpires: null },
  } as Integration;
  state.integrations = [sso, az, gcp, out];
  const s = (o: Record<string, unknown>): Session => ({ id: id(), status: "inactive", ...o }) as unknown as Session;
  state.sessions = [
    s({
      name: "Acme Prod/AdministratorAccess",
      kind: "aws-sso-role",
      region: "us-east-1",
      integrationId: "acme",
      aws: { accountId: "123456789012", roleName: "AdministratorAccess" },
      status: "active",
      favorite: true,
      expires: new Date(Date.now() + 47 * 60e3).toISOString(),
    }),
    s({
      name: "Acme Prod/ReadOnlyAccess",
      kind: "aws-sso-role",
      region: "us-east-1",
      integrationId: "acme",
      aws: { accountId: "123456789012", roleName: "ReadOnlyAccess" },
    }),
    s({
      name: "Acme Dev/PowerUser",
      kind: "aws-sso-role",
      region: "us-east-1",
      integrationId: "acme",
      aws: { accountId: "210987654321", roleName: "PowerUserAccess" },
    }),
    s({
      name: "prod-admin",
    ...
    s({
      ...
      azure: { subscriptionId: "0f1e2d3c-4b5a-6978-8a9b-0c1d2e3f4a5b", tenantId: "t-1" },
      status: "active",
      expires: new Date(Date.now() + 3.2e6).toISOString(),
    }),
...
export const mockApi = {
  ...
  CompleteOnboarding: async () => {
    ...
    if (state.sessions.length === 0) seed();
    emit();
  },
  AddAWSSSO: async (alias: string, startUrl: string, region: string) => {
    const i = { id: id(), alias, cloud: "aws", awsSso: { startUrl, region } } as Integration;
  ...
  WaitSSOLogin: async (ref: string) => {
    await wait(2500);
    const i = state.integrations.find((x) => x.id === ref)!;
    i.awsSso!.tokenExpires = new Date(Date.now() + 8 * 3.6e6).toISOString();
    const added = ["Acme Prod/AdministratorAccess", "Acme Dev/PowerUser", "Acme Sandbox/ReadOnly"].map(
      (name) =>
        ({
          id: id(),
          name,
  ...
  SSOLogout: async (ref: string) => {
    const i = state.integrations.find((x) => x.id === ref);
    if (i?.awsSso) i.awsSso.tokenExpires = null;
    emit();
  },
  SyncSSO: async () => {
    await wait(500);
  ...
  RemoveIntegration: async (ref: string) => {
    state.integrations = state.integrations.filter((i) => i.id !== ref);
    state.sessions = state.sessions.filter((s) => s.integrationId !== ref);
    emit();
  },
  AddAssumeRole: async (v: { name: string; roleArn: string; region: string; sourceRef: string }) => {
    const s = {
      id: id(),
      name: v.name,
      kind: "aws-assume-role",
      region: v.region,
      status: "inactive",
      aws: { roleArn: v.roleArn, sourceSessionId: v.sourceRef },
    } as Session;
    state.sessions.push(s);
    emit();
    return s;
  },
  AddIAMUser: async (v: { name: string; region: string; mfaDevice: string }) => {
    const s = {
      id: id(),
      name: v.name,
      kind: "aws-iam-user",
  ...
  Start: async (ref: string) => {
    ...
    s.expires = new Date(Date.now() + 3.6e6).toISOString();
    emit();
    return creds;
  },
  Stop: async (ref: string) => {
    const s = state.sessions.find((x) => x.id === ref)!;
    s.status = Status.StatusInactive;
    s.expires = null;
    emit();
  },
  ...
  AddAzure: async (alias: string, tenantId: string) => {
    const i = { id: id(), alias, cloud: "azure", azure: { tenantId } } as Integration;
    state.integrations.push(i);
    emit();
    return i;
  },
  AzureLogin: async (ref: string) => {
    await wait(2500);
    const i = state.integrations.find((x) => x.id === ref)!;
    i.azure!.account = "nate@contoso.com";
    const added = [
      {
        id: id(),
        name: "Contoso Production",
  ...
  AzureLogout: async (ref: string) => {
    const i = state.integrations.find((x) => x.id === ref);
    if (i?.azure) i.azure.account = "";
    emit();
  },
  SyncAzure: async () => [],
  GCPStatus: async () => {
    ...
    return {
      ready: gcpReady,
      account: gcpReady ? "nate@example.com" : "",
      loginCommand: "gcloud auth application-default login",
      gcloudFound,
      gcloudPath: gcloudFound ? "/opt/homebrew/bin/gcloud" : "",
  ...
  AddGCP: async (alias: string) => {
    await wait(800);
    const i = { id: id(), alias, cloud: "gcp", gcp: { account: "nate@example.com" } } as Integration;
    state.integrations.push(i);
    const added = [
      {
        id: id(),
  ...
  AddGCPImpersonation: async (v: {
    ...
    const s = {
      ...
      name: v.name,
      kind: "gcp",
      integrationId: v.integrationRef,
      status: "inactive",
      gcp: { projectId: v.projectId, serviceAccount: v.serviceAccount },
    } as Session;
    state.sessions.push(s);
    emit();
    return s;
  ...
  UpdateSettings: async (v: NonNullable<Workspace["settings"]>) => {
    state.settings = { ...v };
    emit();
    return { ...v };
  },
  Info: async () => ({
    version: "0.0.1-dev",
    workspacePath: "~/.config/rolle/workspace.json",
    cacheDir: "~/.cache/rolle/credentials",
  ...
  ImportLeappSessions: async () => {
    await wait(700);
    const added = ["personal-old", "prod-admin-old"].map(
      (name) =>
        ({
          id: id(),
          name,
          kind: name.startsWith("personal") ? "aws-iam-user" : "aws-assume-role",
          region: "us-east-1",
          status: "inactive",
          aws: {},
  ...
  ImportAWSSSO: async (alias: string, startUrl: string, region: string) => {
    await wait(900);
    const i = {
      id: id(),
      alias,
      cloud: "aws",
      awsSso: { startUrl, region, tokenExpires: new Date(Date.now() + 8 * 3.6e6).toISOString() },
    } as Integration;
    state.integrations.push(i);
    const loggedIn = startUrl.includes("engineering");
    const sessions = loggedIn
      ? ["Engineering Prod/AdministratorAccess", "Engineering Dev/PowerUserAccess"].map(
          (name) =>
            ({
              id: id(),
              name,
    ...
    emit();
    return { integration: i, loggedIn, sessions };
  },
  ...
  SetProfile: async (ref: string, profile: string) => {
    const x = state.sessions.find((s) => s.id === ref);
    if (x && x.aws) (x.aws as unknown as { profile?: string }).profile = profile || undefined;
    emit();
  },
  RenameSession: async (ref: string, name: string) => {
    const x = state.sessions.find((s) => s.id === ref);
    if (x) x.name = name;
    emit();
  },
  ...
  CheckForUpdates: async () => {
    ...
    return {
      enabled: true,
      currentVersion: "0.0.1-dev",
      available: avail,
      version: avail ? "0.2.0" : undefined,
      notes: "",
