...
export function ImportDialog({
  ...
  // Imported IAM users; the list rescans and drops them, so the offer to
  // remove their keys lives here, next to the dialog, not inside the list.
        ...
        <DialogContent className="sm:max-w-2xl">
          ...
          {d.loading ? (
            ...
            <FoundList
              ...
              onAWS={(p) =>
                guard(p.startUrl, async () => {
                  const res = await api.ImportAWSSSO(p.alias, p.startUrl, p.region);
                  if (res.loggedIn) {
                    toast.success(`Imported ${p.alias}`, {
                      description: `${res.sessions?.length ?? 0} role${res.sessions?.length === 1 ? "" : "s"} from your AWS CLI sign-in.`,
                    });
              ...
              onLeapp={() =>
                guard("leapp", async () => {
                  ...
                  toast.success(`Imported ${n} session${n === 1 ? "" : "s"} from Leapp`, {
                    description: res.skipped?.length
                      ? `${res.skipped.length} skipped: ${res.skipped[0]}${res.skipped.length > 1 ? "…" : ""}`
                      : undefined,
                  });
              ...
              onImportedUsers={(profiles) => setRemoving({ profiles })}
            />
          )}
        </DialogContent>
      </Dialog>
