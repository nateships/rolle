...
// fake home and routes the AWS SDK path at it.
...
func TestWorkspaceMarksShadowedProfiles(t *testing.T) {
	...
	if len(w.ShadowedProfiles) != 1 || w.ShadowedProfiles["default"] != "~/.aws/credentials" {
	...
	if got := r.ProfileShadow("personal"); got != "~/.aws/credentials" {
	...
	st := r.StaticProfiles()
	if st.Path != "~/.aws/credentials" || len(st.Profiles) != 2 || st.Profiles[0].Imported || st.Profiles[1].Imported {
	...
	if w, _ = r.Workspace(); len(w.ShadowedProfiles) != 2 || w.ShadowedProfiles["personal"] != "~/.aws/credentials" {
