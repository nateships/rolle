...
If `~/.aws/credentials` holds static keys under the same profile name, the AWS credential chain reads those before rolle's `credential_process`. rolle refuses to start a session onto such a profile and marks the profile in the table. Click the mark and choose **Remove from file**, run `rolle session fix-profile <session>`, or give the session another profile name. [Profiles and regions](/aws/profiles-and-regions#static-keys-in-awscredentials) has the details.
