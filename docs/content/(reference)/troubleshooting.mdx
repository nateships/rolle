...
Attach a support bundle: **Settings → About → Support bundle**, or `rolle support` on the command line. It writes a zip to Downloads with your settings, a redacted copy of the workspace, the AWS config, the az and gcloud profile files and tool locations, and the most recent diagnostic lines from every provider. Account IDs, emails, GUIDs, portal hosts, and strings that look like secrets are replaced with placeholders. Keychain entries and cached credentials are not included.
