...
| `rolle session add aws-login --name N --region R [--profile P]` | Add a console login, the flow behind `aws login`. The first start opens the browser |
...
| Command | Purpose |
| --- | --- |
| `rolle start <session> [--mfa-code C] [--no-browser]` | Start a session. `--no-browser` runs the cross-device sign-in of a console login: open the URL elsewhere and paste the code back |
...
| `rolle env <session> [--powershell] [--profile]` | Print credentials as shell exports. `--profile` exports the AWS profile and region instead, so the shell renews through `credential_process` |
