...
rolle [command] [flags]
  ...
  --json         machine-readable output for the list, status, start, and kube commands
...
| `rolle kube token <session>` | Print an `ExecCredential` for kubectl; the contexts `kube add` writes for AKS and GKE run this |

[Kubernetes](/integrations/kubernetes) explains how each cloud authenticates.
