...
rolle [command] [flags]
  ...
  --json         machine-readable output for list, status, and start
  -v, --version  print the version
```

Exit codes: `0` success, `1` error, `3` a sign-in is needed (run `rolle integration login <alias>`), `4` the session or integration does not exist. [Agents and scripts](/agents) shows how to use them.
...
An alias replaces the name Identity Center gives an account or a permission set in every session name. Sync keeps it. No two accounts and no two permission sets can share an alias, and a session renamed by hand keeps its name.

| Command | Purpose |
| --- | --- |
| `rolle alias account <account-id\|name> <alias>` | Name an account; every role of it takes the name |
| `rolle alias role <permission-set> <alias>` | Name a permission set in every account |
| `rolle alias account <account-id\|name> --clear` | Restore the name from Identity Center; `alias role` takes `--clear` too |
| `rolle alias list` | List aliases; `--json` for scripts |
...
| `rolle tag set <name> [--name N] [--color C] [--icon I]` | Change a tag; a new name follows through to every session |
