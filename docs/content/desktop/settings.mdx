...
| Tab | Setting | Meaning |
| --- | --- | --- |
| General | Terminal | Terminal app for **Open terminal**. The system default by default: on macOS, the app that opens `.command` files |
...
| Appearance | Compact rows | One line per session, with less padding |
| Appearance | Group roles by account | On by default. Identity Center roles sit under an account row. Off lists each role on its own |
