...
The row menu also renames a session, sets the AWS profile name, changes the region, and removes the session. The account row's menu hides or unhides every role of the account at once; a sync keeps a hidden account hidden when it finds new roles there. Hiding a session drops it from the favorites.
