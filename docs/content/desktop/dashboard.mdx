...
The row menu also renames a session, sets the AWS profile name, changes the region, and removes the session. For an Identity Center role, the rename dialog's **Apply to this role in every account** switch names the permission set everywhere, and sync keeps that name for new roles. The account row's menu has **Rename account**, which names the account in every role and after every sync, and hides or unhides every role of the account at once; a role that a later sync discovers shows as usual. Hiding a session drops it from the favorites, and starring a hidden session unhides it. Right-click **Hidden** in the sidebar for **Unhide all**.
