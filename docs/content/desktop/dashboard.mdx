...
Each row shows the profile name and region (click to change), the state with a countdown, and actions. A session that ran out on its own reads **Expired** with the time since, in amber, until you start it again, stop it, or an hour passes:
