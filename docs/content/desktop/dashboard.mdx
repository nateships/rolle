...
Tags are groups of your own. The **+** next to **Tags** in the sidebar creates one, with a name, a color, and an icon. Drag a session onto a tag to add it, or use **Tags** in the session's menu. A tag shows on its sessions as a small chip, and its sidebar entry filters the table to them. Drag a tag onto another to reorder the list. The tag's menu edits or removes it; removing a tag takes it off every session. The CLI has the same controls under `rolle tag` and `rolle session tag`.
