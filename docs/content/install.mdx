...
<Tabs>
  ...
  <Tab title="Windows">
    Download `rolle-installer.exe` from the [releases page](https://github.com/nateships/rolle/releases). One installer covers x64 and Arm64. It installs for the current user under `%LOCALAPPDATA%\Programs\rolle`, so it needs no administrator, and neither do updates.
  ...
  <Tab title="Linux">
    ...
    The deb installs the `rolle` command in `/usr/local/bin`. The AppImage carries it: **Install command** in the last onboarding step and in **Settings → General** writes it to `~/.local/bin`, and the app keeps it at its own version after each update. The CLI also ships on its own as `rolle-cli_<version>_linux_<arch>.tar.gz` for machines without the app.
  </Tab>
</Tabs>

## CLI archives

Every release ships `rolle-cli_<version>_<os>_<arch>.tar.gz` (zip on Windows) for `darwin`, `linux`, and `windows` on `amd64` and `arm64`, with a `checksums.txt`. The `rolle-update-*` zips next to them are what the desktop app installs when it updates itself; they are not meant for download. Put the `rolle` binary on your `PATH`.
