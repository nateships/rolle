...
`checksums.txt` also lists `rolle.dmg`, the Windows installer, the AppImage, the deb, and `manifest.json`, so the same check covers a desktop download. Every file in it carries a [build provenance attestation](/security#build-provenance) as well:
