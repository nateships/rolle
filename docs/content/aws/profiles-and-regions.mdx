...
rolle refuses to start a session onto such a profile. The profile column shows an amber mark. Click it, and the profile dialog names the file and offers **Remove from file**, which deletes that section's static keys. The start error offers the same. `rolle session list --json` carries the file as `shadowedBy`, and `rolle session fix-profile <session>` removes the keys.
