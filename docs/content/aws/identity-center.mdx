...
Roles group under their account in the desktop app. Sync adds roles and removes roles you cannot reach. A started role calls `GetRoleCredentials` and renews while the portal sign-in is valid. Fifteen minutes before that sign-in expires, when the portal has active sessions, the desktop app sends a notification with a **Sign in** button.
