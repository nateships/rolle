...
`rolle start <session> --no-browser` prints the sign-in URL instead of opening the browser.

## Permissions

An IAM user or role needs the `SignInLocalDevelopmentAccess` managed policy. The root user needs nothing. An account can restrict the flow with the `arn:aws:signin:{region}:{account}:oauth2/public-client/localhost` resource in an IAM policy.
...
The session learns its account on the first sign-in and shows it under its name. A console login can be the source of an [assume-role session](/docs/aws/assume-role).
