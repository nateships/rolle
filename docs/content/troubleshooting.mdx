...
Attach a support bundle: **Settings → About → Support bundle**, or `rolle support` on the command line. It writes a zip to Downloads with your settings, a redacted copy of the workspace, the AWS config, and the most recent diagnostic lines. Account ids, emails, GUIDs, portal hosts, and anything that looks like a secret are replaced with placeholders. Keychain entries and cached credentials are never included.
