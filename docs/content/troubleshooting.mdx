...
Rolle looks for `gcloud` on your `PATH`, then in the usual install locations: Homebrew, the Google Cloud SDK installer, and version managers such as mise. An app started from the Dock or Start menu does not get your shell's `PATH`, so a `gcloud` that only your shell can see may be missed. Install the SDK with Homebrew or the official installer, or run the sign-in command shown on the Google Cloud connect step and press **Check again**.
