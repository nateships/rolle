...
The updater replaces the app in place, which needs write access to the folder that holds it. A standard account has none in `/Applications` on macOS or in `Program Files` on Windows, so the swap fails after the app has quit; the app itself is untouched and opens again from the Dock or the Start menu. From 0.1.8 the app notices this and asks for an administrator name and password, on macOS through the system prompt and on Windows through UAC, before it replaces itself. Privilege managers such as BeyondTrust or Jamf Connect answer that prompt by policy. If nobody can approve it, on macOS move the app to `~/Applications` and run **Install command** again. The Windows installer is per user from 0.1.9 and needs no administrator; an install in Program Files from an earlier release needs the prompt once, or a fresh run of the installer. The updater writes a log to `$TMPDIR/wails-update-<pid>.log` on macOS and `%TEMP%\wails-update-<pid>.log` on Windows; attach it to an issue with the support bundle.
