...
Rolle marks each profile it writes with a `rolle_session` key. It refuses to touch a section that holds `aws_access_key_id` or other credential keys. A plain `[default]` section that only sets region or output is taken over and restored on stop.
